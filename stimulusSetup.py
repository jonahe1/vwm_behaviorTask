# stimulusSetup.py conducts the counterbancing for this VWM behavioral task.
#
# This file is intended to be run before subjects are present to pre-assign
# stimulus runs during experiment preparation. The pickle files required to
# present the task is generated here, and subject and run numbers entered
# through the console dictate the random seed used to assign trial order
# and relative position/orientation for each object. Pickle generation through
# the console is done as follows:
# python ./stimulusSetup.py %subj %run
...
# ExperimentHandler conducts data saving to record setup conditions
...
# store VWMTrials data structure in a pickle binary file
